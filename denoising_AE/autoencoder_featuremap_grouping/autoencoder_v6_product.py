...
class Autoencoder:
    ...
    def fuse_and_train(self, first_level_output_1d, first_level_output_2d, first_level_output_3d, is_training, suffix = '', dim=3):
        ...
        with tf.variable_scope(var_scope):
            fused_1d = self.cnn_1d_fuse(first_level_output_1d, is_training, '2')
            fused_2d = self.cnn_2d_fuse(first_level_output_2d, is_training, '2')
            ...
            if len(first_level_output_3d)!=0:
                ...
                fused_3d = self.cnn_3d_fuse(first_level_output_3d, is_training, '2')
    ...
    def train_autoencoder(self, rawdata_1d_dict, rawdata_2d_dict, rawdata_3d_dict,
        ...
        second_level_output_2d = dict()
        second_level_output_3d = dict()
        ...
        for grp, data_list in grouping_dict_1d.items():
            ...
            group_fusion_featuremap = cnn_1d_fuse(temp_list, is_training, suffix = '1', output_dim =temp_dim)
        ...
        for grp, data_list in grouping_dict_2d.items():
            # group a list of dataset in a group
            temp_list = [] # a list of feature maps belonging to the same group from first level training
            for ds in data_list:
            ...
            scope_name = '1_'+ grp
            temp_dim = int(len(data_list) / 3) + 1
            group_fusion_featuremap = cnn_2d_fuse(temp_list, is_training, suffix = '1', output_dim =temp_dim)
            second_level_output_2d[grp] = group_fusion_featuremap
            second_order_encoder_list.append(group_fusion_featuremap)
            keys_list.append(grp)

        for grp, data_list in grouping_dict_3d.items():
            # group a list of dataset in a group
            temp_list = [] # a list of feature maps belonging to the same group from first level training
            for ds in data_list:
            ...
            scope_name = '1_'+ grp
            temp_dim = int(len(data_list) / 3) + 1
            group_fusion_featuremap = cnn_3d_fuse(temp_list, is_training, suffix = '1', output_dim =temp_dim)
        ...
        latent_fea = self.fuse_and_train(list(second_level_output_1d.values()),
                            list(second_level_output_2d.values()), list(second_level_output_3d.values()),
                            self.is_training, '2', dim)
    ...
    def inference_autoencoder(self, rawdata_1d_dict, rawdata_2d_dict, rawdata_3d_dict,
                    rawdata_1d_corrupted_dict, rawdata_2d_corrupted_dict, rawdata_3d_corrupted_dict,
                        train_hours,
