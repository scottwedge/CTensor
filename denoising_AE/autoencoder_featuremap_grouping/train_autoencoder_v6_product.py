...
# may 10, 2020
# fusion method use outer product
# grouping only in encoder part
...
import autoencoder_v6_product as autoencoder_v6_individual_init
...
################  read corrputed data ########################
   ...
   '''
    ...
    'group_2': ['airquality', 'POI_transportation', 'transit_stop',
            'slope', 'POI_food'],
    'group_3': ['house_price',  'transit_signals', 'collisions', 'precipitation', 'total_flow_count',
            'POI_business'],
...
####################### city ignorant treatment ################
    ...
    if suffix == '':
        save_path =  './denoise_autoencoder_v6_shuffle_product_'+ 'dim'+ str(dim)  +'/'
    else:
        save_path = './denoise_autoencoder_v6_shuffle_product_'+ 'dim' + str(dim) +'_'+ suffix  +'/'
    ...
    txt_name = save_path + 'denoise_autoencoder_v6_' + 'dim_' + str(dim) +'_'  + timer + '.txt'
