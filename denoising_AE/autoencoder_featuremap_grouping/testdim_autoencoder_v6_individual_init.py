...
def get_dim(group_size):
    if group_size == 0 or group_size == 1:
        return group_size
    else:
        return int(group_size/2)
...
class Autoencoder:
    ...
    def train_autoencoder(self, rawdata_1d_dict, rawdata_2d_dict, rawdata_3d_dict,
        ...
        for grp in list(grouping_dict.keys()):
            group_size = len(grouping_dict[grp])
            print('group: ', grp)
            print('group_size: ', group_size)
            first_level_decode[grp] = self.branching(latent_fea, get_dim(group_size), self.is_training)
