# updated 2020 April 26
# concatenate latent representation from all-to-all autoencoders
# and those from AEs of a certain group
...
def main():
    ...
    if place == "Seattle":
        ...
        economics_latent_rep = np.load(groupwise_latent_rep_path + 'economics_grp.npy')
        transportation_latent_rep = np.load(groupwise_latent_rep_path + 'transportation_grp.npy')
        public_service_latent_rep = np.load(groupwise_latent_rep_path + 'public_service_grp.npy')
        print('weather_latent_rep.shape: ', weather_latent_rep.shape)  # should be [42240, 32, 20, 3]
        group_latent_rep= np.concatenate([weather_latent_rep,economics_latent_rep,
                            transportation_latent_rep, public_service_latent_rep], axis=-1)

        group_latent_series = group_latent_rep[start_train_hour:end_train_hour + test_len + TIMESTEPS,  :,:,:]
