# updated May 3 , 2020
# based on bikeshare prediction fusemodel_v2
# but use all raw datasets used by AE as features
...
def parse_args():
    ...
    parser.add_argument('-use_1d_fea',   type=bool, default=True,
                    action="store", help = 'whether to use 1d features. If use this option, set to True. Otherwise, default False')
    parser.add_argument('-use_2d_fea',    type=bool, default=True,
...
def main():
    ...
    if place == "Seattle":
        ...
        rawdata_1d_dict = {
         'precipitation':  np.expand_dims(weather_arr[start_idx:end_idx,0], axis=1) ,
        'temperature':  np.expand_dims(weather_arr[start_idx:end_idx,1], axis=1) ,
         'pressure':  np.expand_dims(weather_arr[start_idx:end_idx,2], axis=1),
         'airquality': airquality_arr[start_idx:end_idx,:],
        }
