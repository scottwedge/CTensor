# TOY CASE
# try using 24 hours to produce 1-hour latent representation
...
class Autoencoder:
    ...
    def cnn_1d_model(self, x_1d_train_data, is_training, suffix = '', output_dim =3, seed=None):
        ...
        with tf.variable_scope(var_scope):
            ...
            conv2 = tf.layers.average_pooling1d( conv2, 24, 1, padding='valid')
    ...
    def reconstruct_1d(self, latent_fea, dim_1d, is_training):
        ...
        conv3 = tf.layers.conv2d(conv2, 24, 3, padding='same',activation=None)
        conv3 = tf.layers.batch_normalization(conv3, training=is_training)
        conv3 = tf.nn.leaky_relu(conv3, alpha=0.2)
    ...
    #                     feed_dict_all[self.rawdata_3d_tf_y_dict[k]] = temp_batch
                ...
                test_end = rawdata_1d_dict[rawdata_1d_dict.keys[0]].shape[0] -24  # 45984 - 168
