# updated Feb 15, 2020
# updated April 10, self-supervised AE
# predicting next-step instead of reconstructing

# TODO: initialization is still with the normal AE
# should initialized with individualy trained self-supervised version
...
# updated on April 2020. Generate a sequence of 25 steps
# with the last as the prediction target.
def create_mini_batch_1d(start_idx, end_idx,  data_1d):
    ...
    test_data_1d = data_1d[start_idx:end_idx + TIMESTEPS,:]
...
class Autoencoder:
    ...
    def __init__(self, rawdata_1d_dict, rawdata_2d_dict, rawdata_3d_dict,
        ...
        if len(rawdata_1d_dict) != 0:
            ...
            for k, v in rawdata_1d_dict.items():
                ...
                self.rawdata_1d_tf_y_dict[k] = tf.placeholder(tf.float32, shape=[None,  dim])
        ...
        if len(rawdata_3d_dict) != 0:
            for k, v in rawdata_3d_dict.items():
                ...
                self.rawdata_3d_tf_y_dict[k] = tf.placeholder(tf.float32, shape=[None, height, width, 1])
    ...
    # update on April 10. Change the output to predictint next step
    # [None, DAILY_TIMESTEPS, height, width, 1] -> [None, 1, height, width, 1]
    def reconstruct_3d(self, latent_fea, timestep, is_training, suffix = ''):
        ...
        with tf.variable_scope(var_scope):
            ...
            conv2 = tf.nn.leaky_relu(conv2, alpha=0.2)
                ...
                # output of conv3: [None, DAILY_TIMESTEPS, height, width, 1]
            conv3 = tf.layers.conv3d(inputs=conv2, filters= 1, kernel_size=[3,3,3], padding='same', activation=None)
            conv3 = tf.layers.batch_normalization(conv3, training=is_training)
            conv3 = tf.nn.leaky_relu(conv3, alpha=0.2)

            # added on April: [None, 24, height, width, 1] -> [None, height, width, 1]
            # [None, 24, height, width, 1]  -> [None, 24, height, width]
            cnn3d_bn_squeeze = tf.squeeze(conv3, axis = 4)
            # swap axes -> (?, 32, 20, 24) -> [0, 1, 2, 3] -> []
            cnn3d_bn_squeeze = tf.transpose(cnn3d_bn_squeeze, perm=[0,2,3, 1])
            ...
            with tf.name_scope("3d_layer_b"):
                conv5 = tf.layers.conv2d(
                    inputs=cnn3d_bn_squeeze,
                    filters=1,
                    kernel_size=[3, 3], # note, kernel size different from [1x1]
                    padding="same",
                    activation=my_leaky_relu
                    #reuse = tf.AUTO_REUSE
                )
    ...
    # updated April, 2020: [None,24, dim_1d] -> [None, dim_1d]
    def reconstruct_1d(self, latent_fea, dim_1d, is_training, suffix= ''):
        ...
        with tf.variable_scope(var_scope):
            ...
            # added on April: [None,24, dim_1d] -> [None, dim_1d]
            # swap axes -> [None,24, dim_1d]  -> [None, dim_1d, 24]
            cnn4d_bn_squeeze = tf.transpose(conv4, perm=[0,2,1])


            conv5 = tf.layers.conv1d(
                    inputs=cnn4d_bn_squeeze,
                    filters=dim_1d,
                    kernel_size= 3,
                    padding="same",
                    activation=my_leaky_relu
            ...
            for epoch in range(start_epoch, epochs):
                ...
                for itr in range(iterations):
                    ...
                    for k, v in rawdata_1d_dict.items():
                        ...
                        feed_dict_all[self.rawdata_1d_tf_x_dict[k]] = temp_batch[:, 0:TIMESTEPS, :]  # shape=[None,TIMESTEPS, dim]
                    ...
                    for k, v in rawdata_3d_dict.items():
                        ...
                        feed_dict_all[self.rawdata_3d_tf_x_dict[k]] = temp_batch[:, 0:HOURLY_TIMESTEPS, :, :, :] # # shape=[None,HOURLY_TIMESTEPS, height, width, 1])
                ...
                for itr in range(itrs):
                    ...
                    for k, v in rawdata_1d_dict.items():
                        temp_batch = create_mini_batch_1d(start_idx, end_idx, v)
                        test_feed_dict_all[self.rawdata_1d_tf_x_dict[k]] = temp_batch[:, 0:TIMESTEPS, :]
                        test_feed_dict_all[self.rawdata_1d_tf_y_dict[k]] = temp_batch[:, -1, :]
    ...
    #                     print('3d temp_batch.shape: ',temp_batch.shape)
                        test_feed_dict_all[self.rawdata_3d_tf_x_dict[k]] = temp_batch[:, 0:HOURLY_TIMESTEPS, :, :, :]
                        test_feed_dict_all[self.rawdata_3d_tf_y_dict[k]] = temp_batch[:, -1, :, :, :]
                    # is_training: True
...
########
    ...
    #                     print('3d temp_batch.shape: ',temp_batch.shape)
            ...
            # save results to txt
            txt_name = save_folder_path + 'nextstep_infer_AE_v2_latent_rep' +  '.txt'
            with open(txt_name, 'w') as the_file:
                    #the_file.write('Only account for grids that intersect with city boundary \n')
                # the_file.write('epoch\n')
                # the_file.write(str(epoch)+'\n')
