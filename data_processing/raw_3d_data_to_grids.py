'''
Partial data processing for 3D data: 
- Seattle 911 calls
- collision
- crime
- ...
...
DATA_PATH = '/Users/yanan/iSchool/new_bikeshare/SeaTens/'
...
def get_timestamp(date, hour):
    ...
    #datetime_object = datetime.datetime.strptime(time_str, '%Y-%m-%d %H:%M')
#     print time_str
    return time_str
...
def get_total_hour_series(folder_path):
   ...
   # df_name_list = sorted(df_name_list)
...
def intersect_beat_grid(data_path):
    ...
    grid = gpd.read_file('/Users/yanan/iSchool/new_bikeshare/UW_lib/grid_32_20.shp') 
...
#         print('len(intersected_beats): ', len(intersected_beats))
        ...
        for i, row in selected_crime_rows.iterrows():
            ...
            for j in range(24):
    #             total_crime = total_crime + row[j] * area_perc
...
def compose_timeseries(path_to_hourly_tripstarts, grid_demo_intersect):
    ...
    #                     if timestamp not in set(hour_series):
    #                         print('timestamp out of range:', timestamp)
                        if str(i) in cols:
...
#                             hourly_grid_timeseries.loc[timestamp,grid_num] = row[str(i)]
#                         except:
#                             print('error: index not in df', i,grid_num,timestamp)
...
def main():
    ...
    path_to_tripstarts = DATA_PATH + 'daily_911calls/'
    save_path = DATA_PATH + 'daily_grided_911calls/'  # path_to_hourly_tripstarts


    # create hourly grid data
    # hourly_grid_data_feeder(path_to_tripstarts,save_path)

    grid_demo_intersect_path = '/Users/yanan/iSchool/new_bikeshare/UW_lib/whole_grid_32_20_demo_1000_intersect_geodf_2018_corrected.shp'
    grid_demo_intersect = gpd.read_file(grid_demo_intersect_path) # {'init': 'epsg:2163'}
